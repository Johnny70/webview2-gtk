...
Do not edit this file unless you have read [docs/releasing.md](docs/releasing.md).

## [0.6.4]

### Added

- `examples/add-cookie --smoke-proxy-late`: dummy local host proxy, attach, then CUSTOM to TEST-NET — must not paint example.com.

### Fixed

- Later CUSTOM upstream was pass-through: Chromium never sends `Proxy-Authorization` until a 407. Local host proxy 407s the first CONNECT; `BasicAuthenticationRequested` (filtered to `127.0.0.1:<port>`) puts the view id as Basic user ([bug](docs/bugs/done/2026-09-16-custom-upstream-connect-passthrough.md)).

## [0.6.3] - 2026-09-16

### Fixed

- Local host proxy second launch hang: `UserDataFolder` is `wv_<pid>_<tick>_<id>` so a leftover `msedgewebview2` cannot pin the next process to `wv_1`; dead-pid folders are swept ([bug](docs/bugs/done/2026-09-16-userdatafolder-second-run-hang.md)).
