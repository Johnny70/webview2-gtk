...
## Automation(fill via external driver)

WebKitGTK-shaped **setup** (`WebContext.set_automation_allowed`, controlled view, `automation_started`, `WEBKIT_INSPECTOR_SERVER`) is emulated on Windows. **Click/sendKeys are not `WebView` methods** — use an external CDP/WebDriver client against the inspector port.
